...
meanPlotEvolutionPT <- function(instance, indicator, algorithmsNames){
	# Read data file
	algorithms <- c()

#remove <- c(1:10) * 5 #50%
remove <- c(1:19) * 5 #100%
#remove <- c(15:19) * 5 #100%
	...
	algorithm1mean <- algorithm1mean[remove]
	#FIRST ALGORITHM
	plot(algorithm1mean, type="o", pch=21, lty=1, col=plot_colors[1],  
		xaxt='n',#this axis will be described later
		ylim=c(min_y,max_y), # Make y axis 
		ann=FALSE)

	l = seq(0,99,25)# points where is to plot
	lab = l+zoommin		# labels 


	#axis(1,at=c(1,5,10),labels=c(1,25,50))
	axis(1,at=c(1,5,10,15,19),labels=c(1,25,50,75,100))
	#axis(1,at=c(15,17,19),labels=c(75,87.5,100))
	...
	for(i in 2:length(algorithmsNames)){
		...
		algorithmMean <- algorithmMean[remove]
		lines(algorithmMean, type="o", pch=pch, lty=1, col=plot_colors[i])
		pch<-pch+1
	}
	...
	title(ylab= paste("Valor do indicador de qualidade ",indicator,sep=""))
	# Create a legend
	legend("bottomright", algorithmsNames, cex=0.6, col=plot_colors, pch=21:(21+length(algorithmsNames)), lty=rep(1,length(algorithmsNames)));
}



meanPlotEvolutionFinalPT <- function(instance, indicator, algorithmsNames){
	# Read data file
	algorithms <- c()

remove <- (c(0:9) * 5)+50 #50%
#remove <- c(1:19) * 5 #100%
	...
	axis(1,at=c(1,5,9),labels=c(50,75,100))
	#axis(1,at=c(1,5,10,15,19),labels=c(1,25,50,75,99))
